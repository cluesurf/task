...
export type DisassembleGhidraNodeInput = {
  ...
  script?: string                  // path to a custom post-script
  ghidraHome?: string
  projectDir?: string
  projectName?: string
  keepProject?: boolean
...
export type DisassembleGhidraNodeOutput = { file: { path: string } }

export async function disassembleGhidraNode(
  src: DisassembleGhidraNodeInput,
): Promise<DisassembleGhidraNodeOutput> {
  const profile = src.profile ?? 'functions'
  const out =
    src.output ??
    `${src.input}.${profile === 'calls' ? 'dot' : 'txt'}`
  await fs.mkdir(path.dirname(out), { recursive: true })

  const headless = await locateAnalyzeHeadless(src.ghidraHome)

  const projectDir = src.projectDir ??
    (await fs.mkdtemp(path.join(os.tmpdir(), 'task-ghidra-')))
  const projectName = src.projectName ?? 'task-disasm'

  const scriptPath = src.script ?? (await materializePostScript(profile, out))
  ...
  const args = [
    ...
    '-import', src.input,
    '-overwrite',
    '-deleteProject',          // tear down the project after we're done
    '-scriptPath', scriptDir,
    '-postScript', scriptName,
  ]
  if (src.quiet) args.push('-log', '/dev/null')

  if (src.keepProject) {
    // Strip the auto-delete flag if the user wants to inspect the
    // project in the GUI later.
    const i = args.indexOf('-deleteProject')
    if (i >= 0) args.splice(i, 1)
  }

  try {
    await run(headless, args, { quiet: src.quiet })
  } finally {
    if (!src.script) await fs.unlink(scriptPath).catch(() => undefined)
...
async function locateAnalyzeHeadless(home?: string): Promise<string> {
  ...
  if (process.env.GHIDRA_INSTALL_DIR) {
    candidates.push(
      path.join(process.env.GHIDRA_INSTALL_DIR, 'support', 'analyzeHeadless'),
    )
  }
...
async function materializePostScript(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'task-ghidra-script-'))
...
const SCRIPT_BODIES: Record<GhidraProfile, (out: string) => string> = {
  functions: out => HEADER + `
...
function py(s: string): string {
  // Single-quote with escapes for Jython source.
    ...
    child.on('error', err => {
      reject(new Error(
        (err as NodeJS.ErrnoException).code === 'ENOENT'
          ? `disassemble ghidra: \`${cmd}\` not found. Install ghidra or pass --ghidra-home.`
          : `disassemble ghidra: ${cmd} failed — ${err.message}`,
      ))
    })
    child.on('exit', code => {
      if (code === 0) resolve()
      else reject(new Error(`disassemble ghidra: analyzeHeadless exited with code ${code}`))
    })
  })
}
