/**
 * `task set environment` — upsert a `KEY=VALUE` entry into a
...
export type SetEnvironmentNodeInput = {
  key: string
  value: string
  file?: string
}

export type SetEnvironmentNodeOutput = {
  file: string
  key: string
...
export async function setEnvironmentNode(
  ...
  let existing = ''
  let action: SetEnvironmentNodeOutput['action'] = 'created'
