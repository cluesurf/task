...
# or: make/deck/linux/docker-build.sh rpm   # any host with Docker
```
...
it at `https://deck.clue.surf/task/rpm-code`.

## Install (end user)

```sh
sudo rpm --import https://deck.clue.surf/task/rpm/pubkey.asc
sudo tee /etc/yum.repos.d/cluesurf.repo <<'EOF'
[cluesurf]
name=ClueSurf
baseurl=https://deck.clue.surf/task/rpm
enabled=1
gpgcheck=1
gpgkey=https://deck.clue.surf/task/rpm/pubkey.asc
EOF
